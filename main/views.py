...
def index(request):
    ...
    context = {"random_service": random_service}
...
def finalize_appointment(request):
    if request.method == "GET":
        ...
        if Appointment.objects.filter(
            doctor_id=doctor_id, date=selected_date, time=selected_time
        ).exists():
            return render(
                request,
                "main/error.html",
        ...
        return render(
            request, "main/appointment_success.html", {"appointment": appointment}
        )
